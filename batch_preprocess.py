        ...
        try:
            ...
            with open(filepath, 'rb') as file:
                ...
                if response.status_code == 200:
                    ...
                    # Debug log of full response (helpful to check saved_path/document fields)
                    log_message(log_file, f"  → API response: {json.dumps(result, ensure_ascii=False)[:1000]}")
                    chunk_count = len(result.get('chunks', []))
                    saved_path = result.get('saved_path') or result.get('document', {}).get('saved_path') or "unknown"
                    log_message(log_file, f"  ✅ Success: {chunk_count} chunks created; saved_path: {saved_path}")
                    return True
                else:
